...
static int check_snprintl_trunc(
  log_t* log, char* buf, size_t size, char* printedlog, size_t sizeofprintedlog)
{
	int want;
...
int test_snprintl()
{
	...
	check_snprintl_trunc(
	  &EXPECTED1, buf1, 212, PRINTEDLOG1, sizeof(PRINTEDLOG1));
	check_snprintl_trunc(
	  &EXPECTED1, buf1, 211, PRINTEDLOG1, sizeof(PRINTEDLOG1));
	check_snprintl_trunc(
	  &EXPECTED1, buf1, 150, PRINTEDLOG1, sizeof(PRINTEDLOG1));
	check_snprintl_trunc(
	  &EXPECTED1, buf1, 10, PRINTEDLOG1, sizeof(PRINTEDLOG1));
