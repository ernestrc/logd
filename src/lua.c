...
static int lua_load_init_modules(lua_t* l)
{
	...
	char* config_lua_buf = NULL;
	int ret = 0;

	if ((init_lua_buf = calloc(1, init_lua_len + 1)) == NULL ||
	  (config_lua_buf = calloc(1, config_lua_len + 1)) == NULL ||
	...
	memcpy(config_lua_buf, config_lua, config_lua_len);

	if ((ret = luaL_dostring(l->state, luvipath_lua_buf)) ||
	  (ret = luaL_dostring(l->state, config_lua_buf)) ||
